...
class HTTP2ConnectionPlaintext(HTTP2Connection):
    def connect(self) -> None:
        """
        Connect to the host on the given port and attempt an HTTP/2 upgrade.
        """
        with self._h2_conn as conn:
            # Create an HTTP/1.1 connection and reuse the socket.
            ...
            if response.status != 101:
                raise ConnectionError("Upgrade to HTTP/2 failed: %s" % response.status)
