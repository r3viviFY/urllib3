...
from urllib3.util.util import to_str  # type: ignore[import-untyped]

from .._base_connection import _TYPE_BODY, _ResponseOptions
from .._collections import HTTPHeaderDict
from ..connection import HTTPSConnection, _get_default_user_agent, HTTPConnection
...
class _LockedObject(typing.Generic[T]):
    ...
    def __exit__(
        ...
        self.lock.release()
class HTTP2ConnectionNoTLS(HTTPConnection):
    def __init__(
    ...
    def connect(self) -> None:
        ...
        with self._h2_conn as conn:
            dummyConnection = HTTPConnection(self.host, self.port)
            dummyConnection.connect()
            dummyConnection.request("GET", "/", headers={
                "Connection": "Upgrade",
                "Upgrade": "h2c",
                "HTTP2-Settings": conn.initiate_upgrade_connection(),
            })
            if dummyConnection.getresponse().status == 101:
                # Http2 upgrade successful and close our current socket and replace it
                self.sock.close()
                self.sock = dummyConnection.sock
            else:
                raise ConnectionError("HTTP/2 upgrade failed")
